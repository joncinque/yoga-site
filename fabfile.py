...
env.proj_app = real_project_name("yoga-site")
...
env.manage = "%s/bin/python3 %s/manage.py" % (env.venv_path, env.proj_path)
env.domains = conf.get("DOMAINS", [conf.get("LIVE_HOSTNAME", env.hosts[0])])
env.domains_nginx = " ".join(env.domains)
env.domains_regex = make_domains_regex(env.domains)
env.domains_python = ", ".join(["'%s'" % s for s in env.domains])
env.ssl_disabled = "#" if conf.get("SSL_DISABLED", True) else ""
env.site = conf.get("SITE", None)
...
env.email_pass = conf.get("EMAIL_HOST_PASSWORD", "")
env.mailchimp_user = conf.get("MAILCHIMP_USERNAME", "")
env.mailchimp_pass = conf.get("MAILCHIMP_PASSWORD", "")
env.mailchimp_key = conf.get("MAILCHIMP_API_KEY", "")
env.mailchimp_list = conf.get("MAILCHIMP_AFA_LIST", "")
...
def make_domains_regex(domains):
    final_domains = []
    for domain in domains:
        if domain.startsWith('.'):
            final_domains.append('www'+domain)
            final_domains.append(domain[1:])
        else:
...
def python(code, show=True):
    ...
    full_code = 'python3 -c "%s%s"' % (setup, code.replace("`", "\\\`"))
    with project():
        if show:
            print_command(code)
...
def install():
    ...
    apt("python3-dev python3-setuptools python3-pip python3-venv " # python stuff
        "nginx memcached supervisor git-core ") # others
    ...
    print(green("Successfully set up git, pip, venv, nginx, "
...
def create():
    ...
    with cd(env.venv_home):
        if exists(env.proj_name):
            if confirm("Virtualenv already exists in host server: %s"
                ...
                abort()
        run("python3 -m venv %s" % env.proj_name)
        ...
        with cd(conf_path):
            ...
            if not exists(crt_file) and not exists(key_file):
                try:
                    ...
                    sudo("certbot certonly --webroot --webroot-path=%s -d %s -d www.%s" % (env.proj_path, env.site, env.site))
                else:
    ...
    with project():
        ...
        if env.reqs_path:
            pip("-r %s/%s" % (env.proj_path, env.reqs_path))
        pip("gunicorn setproctitle django-compressor python-memcached ")
        # Bootstrap the DB
        manage("makemigrations")
        manage("makemigrations theme")
        manage("migrate")
